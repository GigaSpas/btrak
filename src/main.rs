...
fn main() {
    ...
    if args.len() == 1 {
        ...
        println!("{}", day.display(&true, &true, &vec![])); exit(0)
    }
...
Options:
    ...
    if  args[1] == "display"{
        ...
        if args.len() == 3{
            ...
            let date = curr_date;
            let month = read_month_file(date).unwrap_or(BudgetMonth::new(date.month0() as u8 + 1, Vec::new() ));
            if !display_day {
                println!("{}", month.display(date.month0() as u32 + 1, &income, &true, &tags));
                exit(0)
            }
            let day = month.get_day(date.day0() as u8 + 1);
            println!("{}", day.display(&true, &true, &tags));
            exit(0)
        ...
        if args.len() == 4{
            ...
            let date;
            date = chrono::NaiveDate::parse_from_str(&args[3], "%d.%m.%Y").expect("Could not parse the date make sure format is \"DD.MM.YYYY\"");
            let month = read_month_file(date).unwrap_or(BudgetMonth::new(date.month0() as u8 + 1, Vec::new() ));
            if !display_day {
                println!("{}", month.display(date.month0() as u32 + 1, &income, &true, &tags));
                exit(0)
            ...
            let day = month.get_day(date.day0() as u8 + 1);
            println!("{}", day.display(&income, &true, &tags));
            exit(0)
        }
    }
    ...
    if args[1] == "add"{
        ...
        io::stdin().read_line(&mut name).expect("Failed to read input"); println!("Enter a description(optional)");
    ...
    let date;
    date = chrono::NaiveDate::parse_from_str(&args[1], "%d.%m.%Y").expect("Could not parse the date make sure format is \"DD.MM.YYYY\"");
    let month = read_month_file(date).unwrap_or(BudgetMonth::new(date.month0() as u8 + 1, Vec::new() ));
    let day = month.get_day(date.day0() as u8 + 1);
    println!("{}", day.display(&true, &true, &vec![]));
    exit(0)
